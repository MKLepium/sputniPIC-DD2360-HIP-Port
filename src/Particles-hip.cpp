...
int mover_PC_gpu(struct particles* part, struct EMfield* field, struct grid* grd, struct parameters* param)
{
    ...
    hipMalloc(&gpu_part, sizeof(particles));
    hipMalloc(&gpu_field, sizeof(EMfield));
    hipMalloc(&gpu_grd, sizeof(grid));
    hipMalloc(&gpu_param, sizeof(parameters));

    hipMemcpy(gpu_part, part, sizeof(particles), hipMemcpyHostToDevice);
    hipMemcpy(gpu_field, field, sizeof(EMfield), hipMemcpyHostToDevice);
    hipMemcpy(gpu_grd, grd, sizeof(grid), hipMemcpyHostToDevice);
    hipMemcpy(gpu_param, param, sizeof(parameters), hipMemcpyHostToDevice);
    // start subcycling
    for (int i_sub=0; i_sub < part->n_sub_cycles; i_sub++){
        // move each particle with new fields
        int threadsPerBlock = 256;
        int blocksPerGrid = (part->nop + threadsPerBlock - 1) / threadsPerBlock;
        hipLaunchKernelGGL(mover_GPU, dim3(blocksPerGrid), dim3(threadsPerBlock), 0, 0, gpu_part, gpu_field, gpu_grd, gpu_param);
        hipDeviceSynchronize();
    } // end of one particle
    hipMemcpy(part, gpu_part, sizeof(particles), hipMemcpyDeviceToHost);
    hipMemcpy(field, gpu_field, sizeof(EMfield), hipMemcpyDeviceToHost);
    hipMemcpy(grd, gpu_grd, sizeof(grid), hipMemcpyDeviceToHost);
    hipMemcpy(param, gpu_param, sizeof(parameters), hipMemcpyDeviceToHost);
